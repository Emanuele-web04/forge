...
writeFileSync(fixture, `<!doctype html><html class="dark"><meta charset="utf-8">
...
const app = await _electron.launch({ executablePath: electronPath, args: [entry], env: {...process.env, ELECTRON_RUN_AS_NODE: ""} });
...
try {
 for(let repeat=0;repeat<3;repeat++){
  for(const variant of repeat%2 ? ["optimized","baseline"] : ["baseline","optimized"]){
   ...
   if(variant === "optimized") {
    ...
    const finalCss=readdirSync(finalAssets).filter(f=>f.endsWith(".css")).sort((a,b)=>statSync(join(finalAssets,b)).size-statSync(join(finalAssets,a)).size)[0];
    await page.evaluate(href=>{document.querySelector('link[rel="stylesheet"]').href=href;},pathToFileURL(join(finalAssets,finalCss)).href);
    await page.waitForFunction(()=>getComputedStyle(document.querySelector('.shimmer')).animationTimingFunction.includes('40'));
   }
   const timing = await page.evaluate((variant) => {
    for(const el of document.querySelectorAll(variant === "optimized" ? ".animate-spin-stepped,.shimmer" : ".animate-spin-stepped")) for(const animation of el.getAnimations()) animation.startTime=0;
    return [...document.querySelectorAll(".animate-spin-stepped,.shimmer")].map(el => ({className:el.getAttribute("class"), computedTiming:getComputedStyle(el).animationTimingFunction, animations:el.getAnimations().map(a=>({startTime:a.startTime,...a.effect.getTiming()}))}));
   },variant);
   ...
   const metrics = await app.evaluate(({app})=>app.getAppMetrics().map(m=>({pid:m.pid,type:m.type,cpu:m.cpu,memory:m.memory})));
   const elapsedMs=performance.now()-start; const cpuSeconds=Object.fromEntries(metrics.map(m=>[m.type,m.cpu.cumulativeCPUUsage-(before.find(p=>p.pid===m.pid)?.cpu.cumulativeCPUUsage??m.cpu.cumulativeCPUUsage)])); const row={repeat,variant,elapsedMs,timing,metrics,cpuSeconds};samples.push(row);
 ...
 const reducedMotion=await page.evaluate(()=>[...document.querySelectorAll('.animate-spin-stepped,.shimmer')].map(el=>({className:el.getAttribute('class'),animations:el.getAnimations().length,color:getComputedStyle(el).color,textFill:getComputedStyle(el).webkitTextFillColor})));
 if(reducedMotion.some(m=>m.animations!==0)||reducedMotion[1].color!==reducedMotion[1].textFill)throw Error('Reduced motion regression');
 writeFileSync(join(artifacts,'status-reduced-motion.json'),JSON.stringify(reducedMotion,null,2));
