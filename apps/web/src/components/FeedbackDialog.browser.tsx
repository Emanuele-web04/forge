...
const draftButton = () =>
  page.getByRole("button", { name: /Draft a GitHub issue|Opening thread/ });

describe("FeedbackDialogForm busy state", () => {
  it.each([
    { label: "isSending", props: { isSending: true } },
    { label: "isDraftingIssue", props: { isDraftingIssue: true } },
  ])("disables submit and draft buttons while $label", async ({ props }) => {
    ...
    await view.rerender(
      <FeedbackDialogForm
        ...
        isDraftingIssue={props.isDraftingIssue}
        onSubmit={noopSubmit}
        onDraftGithubIssue={noopDraft}
      />,
    );
