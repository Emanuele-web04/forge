...
export const SYNARA_HARNESS_POLICY_VERSION = "2026-08-02.1";
...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "Use the device_* tools whenever the user refers in any language to the iOS simulator, the device pane, or running their app on a simulator. They are the canonical control surface for it: do not shell out to xcrun simctl, idb, or an OS-automation skill instead. Start with device_list to get a udid, and build the app with your own shell before device_install; Synara never builds it for you.",
        "Locate elements with device_describe_ui and tap the frames it reports; those are device points, not screenshot pixels. Use device_screenshot only when the pixels themselves matter. Tap a field before device_type, which does not focus anything by itself.",
        'If device_boot returns kind "boot-limit-reached", Synara has hit its cap on simulators it booted: relay the listed devices and ask the user which to shut down rather than retrying. If a device tool reports DeviceApprovalRequired, the action was refused before it ran because this session has no approval gate: explain that the user must do it from the device pane and do not retry it.',
