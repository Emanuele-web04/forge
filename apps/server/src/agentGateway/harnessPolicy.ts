...
export const SYNARA_HARNESS_POLICY_VERSION = "2026-08-03.2";
...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "Before every tap, call device_describe_ui and aim at the target element: tap its activationPoint when the node has one, and the centre of its frame otherwise. Those are device points, not screenshot pixels, and guessing coordinates is the most common way these tools appear to do nothing. A control such as a switch, checkbox, or stepper is merged into its whole row, so the row's frame centre is dead space and only the activationPoint hits the control.",
        ...
        "The accessibility tree only changes when the screen does, so an unchanged tree after a tap means the tap missed, not that it silently worked: re-read the tree and tap the corrected point rather than continuing as if it landed. Scroll with device_swipe to bring an off-screen row into the tree before tapping it. An input tool that reports HID events were not delivered to the simulator did nothing at all; the server already retried once, so surface that failure rather than continuing. Never report success you have not observed in the tree.",
