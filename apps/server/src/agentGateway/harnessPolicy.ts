...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "For Kanban coordination use the synara_* board tools: synara_read_kanban_board for the durable board, synara_create_kanban_task to open a tracked task, and synara_move_kanban_card to start or finish a card's work. Target columns are derived from live thread state, not stored: move-card dispatches start/settle (interrupt) to the thread rather than editing a column. An Awaiting-you card is waiting on the human (approval or input) and must not be force-moved.",
