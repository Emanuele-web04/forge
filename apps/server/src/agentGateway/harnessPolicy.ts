...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "Use the device_* tools autonomously for any request to run, test, check, demo, debug, or interact with an iOS app or simulator, in any language, whether or not the user names a tool. They are the canonical control surface: never drive the simulator with xcrun simctl, idb, AppleScript, or an OS-automation skill, because the user watches the pane these tools stream and shell commands bypass that view entirely.",
        "Workflow: device_list for a udid, then device_boot (or reuse one already booted). If there is an app to build, build it in your own shell with xcodebuild or the project's tool — Synara never builds for you — then device_install and device_launch, which open the pane on the device you are driving. For a system app already on the device, launch it by bundle id (Settings is com.apple.Preferences).",
        "Before every tap, call device_describe_ui and tap the centre of the target element's reported frame; those are device points, not screenshot pixels, and guessing coordinates is the most common way these tools appear to do nothing. After each action, call device_describe_ui again to confirm the screen actually changed, and use device_screenshot to verify visually or to show the user a result.",
        "The accessibility tree only changes when the screen does, so an unchanged tree after a tap means the tap missed, not that it silently worked: re-read the tree and tap the corrected frame rather than continuing as if it landed. Scroll with device_swipe to bring an off-screen row into the tree before tapping it. An input tool that reports HID events were not delivered to the simulator did nothing at all; the server already retried once, so surface that failure rather than continuing. Never report success you have not observed in the tree or a screenshot.",
