...
const entry = {
  ...
  author: "Ada",
  author_uuid: "{ada}",
  source_branch: "feature/gateway",
  destination_branch: "master",
  created_on: "2026-09-03T15:06:03.529364+00:00",
...
describe("deployed Paraty MCP contract", () => {
  it("translates internal list arguments to the MCP tool schema", async () => {
    expect(
      await Effect.runPromise(
        binding.operations.list.encode({
          workspace: "paraty",
  ...
  it.each(["detail", "diff", "comments"] as const)(
    "translates %s repository arguments",
    async (operation) => {
      const pagination = operation === "comments" ? { page: 1, pagelen: 50 } : {};
      expect(
        await Effect.runPromise(
          binding.operations[operation].encode({
            workspace: "paraty",
            repository: "payment-seeker",
            pull_request_id: 2286,
            ...pagination,
          }),
        ),
      ).toEqual({
  ...
  it("decodes structured summaries and retains pagination and skipped entries", async () => {
    const result = await Effect.runPromise(
      binding.operations.list.decode({
        structuredContent: {
          workspace: "paraty",
          repo_slug: "payment-seeker",
          page: 1,
          pagelen: 50,
          pull_requests: [entry, { id: "bad" }],
          has_more: true,
          next_page: 2,
          total_count: null,
          skipped_count: 3,
    ...
    expect(result).toMatchObject({
      values: [{ id: 2286, draft: true, author: { display_name: "Ada" } }],
      malformedCount: 4,
    });
  ...
  it("decodes an explicit comments page without requiring size", async () => {
    expect(
      await Effect.runPromise(
        binding.operations.comments.decode(
          json({
            pagelen: 50,
            page: 1,
            values: [],
  ...
  it("accepts raw diff text and exposes server truncation", async () => {
    ...
    expect(
      await Effect.runPromise(
        binding.operations.diff.decode({ content: [{ type: "text", text: patch }] }),
      ),
    ).toEqual({ patch, truncated: false });
    expect(
      await Effect.runPromise(
        binding.operations.diff.decode({
