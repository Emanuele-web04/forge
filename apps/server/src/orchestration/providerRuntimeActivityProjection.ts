...
import { activityDataField, boundActivityData, stringifyJsonLike } from "../activityData.ts";
import {
...
export function projectProviderRuntimeActivities(
  ...
  switch (event.type) {
    ...
    case "event.unmapped": {
      ...
      // Bound and redact again at the durable activity boundary. This also
      // protects replay of older journal rows written before ingress hardening.
      return [
        {
          id: event.eventId,
          createdAt: event.createdAt,
          ...
          payload: toActivityPayload({
            ...
            ...(rawData === undefined
              ? {}
              : activityDataField(sanitizeUnmappedProviderData(rawData))),
          }),
          turnId: toTurnId(event.turnId) ?? null,
          ...maybeSequence,
        },
