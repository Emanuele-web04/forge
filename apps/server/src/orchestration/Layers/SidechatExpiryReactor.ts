...
const lastActivityAtMs = (thread: OrchestrationThread): number =>
...
export const makeSidechatExpiryReactor = Effect.gen(function* () {
  const orchestrationEngine = yield* OrchestrationEngineService;
  const providerService = yield* ProviderService;
  const knownThreadIds = new Set<ThreadId>();

  let timer: ReturnType<typeof createSidechatExpiryTimer<NodeJS.Timeout>>;
  ...
  const refreshAfterExpiryFailure = (threadId: ThreadId, expectedLastActivityAtMs: number) =>
    Effect.gen(function* () {
      ...
      const persistedActivityAtMs = lastActivityAtMs(thread);
  ...
  timer = createSidechatExpiryTimer<NodeJS.Timeout>({
    now: Date.now,
    schedule: (callback, delayMs) => setTimeout(callback, delayMs),
    cancel: clearTimeout,
    onExpire: (threadId, expectedLastActivityAtMs) => {
      Effect.runFork(expireSidechat(threadId, expectedLastActivityAtMs));
    },
  });
      ...
      switch (event.type) {
        ...
        case "thread.sidechat-activity-recorded":
          timer.recordActivity(
            ...
            parseTimestamp(event.payload.lastActivityAt) ?? Date.now(),
          );
          return;
        case "thread.turn-start-requested":
          timer.setRunning(
            event.payload.threadId,
            true,
            parseTimestamp(event.payload.createdAt) ?? Date.now(),
          );
          return;
        case "thread.session-set":
          timer.setRunning(
            ...
            parseTimestamp(event.payload.session.updatedAt) ?? Date.now(),
          );
          return;
        ...
        case "thread.unarchived": {
          ...
          timer.restore({
            threadId: thread.id,
            lastActivityAtMs: lastActivityAtMs(thread),
