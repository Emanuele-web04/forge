/**
 * CopilotAdapterLive - GitHub Copilot CLI over the standard Agent Client Protocol.
 *
 * Provider-specific behavior is deliberately limited to process startup, auth,
 * and model discovery in CopilotAcpSupport. Session lifecycle, permissions,
 * elicitation, streaming, cancellation, and recovery stay ACP-standard here.
 */
...
export function makeCopilotAdapter(defaultSettings: CopilotAcpRuntimeSettings = {}) {
  return Effect.gen(function* () {
    ...
    const publish = (
      ctx: Pick<CopilotSessionContext, "lifecycleGeneration">,
    ...
    const publishContext = (
      ctx: CopilotSessionContext | undefined,
      lifecycleGeneration: string | undefined,
    ): Pick<CopilotSessionContext, "lifecycleGeneration"> =>
    ...
    const stopSessionInternal = (ctx: CopilotSessionContext, options: StopSessionOptions = {}) =>
      Effect.gen(function* () {
    ...
    const installInteractionHandlers = (
      ...
      getContext: () => CopilotSessionContext | undefined,
          ...
          Effect.gen(function* () {
            ...
            yield* publish(
              ...
              makeAcpRequestOpenedEvent({
                ...
                permissionRequest,
                detail: permissionRequest.detail ?? "Copilot requested permission.",
                args: params,
                source: "acp.jsonrpc",
    ...
    const waitForAcpQueuedTurnEventsDrained = (ctx: CopilotSessionContext) =>
      Effect.gen(function* () {
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const effectiveSettings: CopilotAcpRuntimeSettings = {
            binaryPath: configured?.binaryPath ?? defaultSettings.binaryPath,
          };
          ...
          if (selectedModel) {
            yield* acp.setModel(selectedModel).pipe(
              Effect.mapError((error) =>
          ...
          yield* Effect.gen(function* () {
            ...
            Effect.onError(() =>
              stopSessionInternal(ctx, {
                exitKind: "error",
                reason: "Failed to publish Copilot session startup events.",
              }),
            ),
          );
          ctx.exitWatcherFiber = yield* acp.awaitExit.pipe(
            Effect.andThen(
              withThreadLock(
                ...
                stopSessionInternal(ctx, {
                  exitKind: "error",
                  reason: "GitHub Copilot process exited unexpectedly.",
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          if (requestedMode && requestedMode !== ctx.modeState?.currentModeId) {
            yield* ctx.acp.setMode(requestedMode).pipe(
              Effect.mapError((error) =>
          ...
          if (turnModel && turnModel !== ctx.session.model) {
            yield* ctx.acp.setModel(turnModel).pipe(
              Effect.mapError((error) =>
                mapAcpToAdapterError(PROVIDER, input.threadId, "session/set_config_option", error),
              ),
            );
    ...
    const getComposerCapabilities: NonNullable<CopilotAdapterShape["getComposerCapabilities"]> = () =>
      Effect.succeed({
        provider: PROVIDER,
      ...
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          const runtime = yield* makeCopilotAcpRuntime({
            childProcessSpawner,
            copilotSettings: {
              binaryPath: input.binaryPath ?? defaultSettings.binaryPath,
            },
            cwd,
