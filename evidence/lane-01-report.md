...
| One-frame scroll detachment (16 ms stream + one wheel, scrollTop decreases next frame, no re-snap for 60 frames) | FAIL under load — needs quiet-machine rerun (report-only) | `MessagesTimeline.scrollOwnership.browser.tsx` "lets a single wheel up detach auto-follow and never re-snaps for 60 frames" failed in SETUP (`startStreamingAndFollow`: distance-from-bottom 513 px > 120 px tolerance within 5 s) with machine load average 16–25 (concurrent lanes' suites). The detachment assertions never ran. File is byte-identical to PR head (upstream never touched it), so the merge cannot be the cause. |
...
Headless browser harness run (playwright chromium, `bun run test:browser` with `VITEST_BROWSER_API_PORT=51301`): 105 passed / 4 failed (109 total). The 4 failures are frame-timing assertions under load; two different runs produced different failure assertions (flake signature). The other two failing files/tests: `ChatView.browser.tsx` estimator-parity tests ("keeps near-cap composer work bounded…", "moves a sent message to its anchor once…") — both pre-existing tests present at merge-base `d01b0c800`. Retry on a quiet machine pending.
...
4. **4 browser tests could not be verified under load** (load average 16–25 from concurrent lanes); 2 of the 4 files are byte-identical to the PR head. Quiet-machine rerun pending at report time; result recorded below if completed.
...
| (pending)   | Findings doc: append 30-minute trace measurements                                                                                 | see final commit                                                 |
...
1. None required for correctness of the merge — all web logic tests pass; blank-flash fixture passes.
2. If the 4 load-flaky browser tests still fail on a quiet machine, investigate `startStreamingAndFollow` tolerance (120 px / 5 s) and the estimator-parity hydration timeout (90 s) for load resilience — files: `MessagesTimeline.scrollOwnership.browser.tsx`, `MessagesTimeline.tailAnchor.browser.tsx`, `ChatView.browser.tsx` (test files only).
