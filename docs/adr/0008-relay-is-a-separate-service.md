...
Considered and rejected (2026-08-13, after studying t3code): a control-plane-only relay that provisions per-host Cloudflare Tunnels so data never touches our service. Proven in their production, but it hard-couples the data path to Cloudflare and per-user tunnel limits; we keep the data plane on infrastructure we own, accepting the bandwidth cost and splice/backpressure work.
