...
This refines the "relay is a dumb pipe" invariant: the invariant applies to *spliced session traffic* (opaque frames, never parsed). The *control* sockets — host dial, client control — do speak a small relay protocol (presence, grant presentation, splice signaling). Data sockets remain opaque end-to-end.
