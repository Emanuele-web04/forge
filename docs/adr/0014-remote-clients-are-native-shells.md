...
Rejected: a cloud-hosted web client (new deployable, browser-keystore and origin work) and t3code-tunnel-style HTTP forwarding through the relay (grant semantics would have to cover HTTP; the relay stops being WS-only). Either can be added later without changing the host model — the host already serves its own UI locally.
